...
class Translator(object):
    ...
    def _fast_translate_batch(self,
        ...
        # tag_features = tag_src.matmul(self.model.tag_embeddings.weight[1:1+tag_src.size(2)])
        tag_features = self.model.tag_embeddings.matmul(tag_src)
        src_features = src_features + tag_features
        ...
        for step in range(max_length):
            decoder_input = alive_seq[:, -1, 0].view(1, -1)
            decoder_tag = alive_seq[:, -1, 1].view(1, -1)

            # Decoder forward.
            decoder_input = decoder_input.transpose(0, 1)
            decoder_tag = decoder_tag.transpose(0, 1)
            decoder_tag.masked_fill_(decoder_tag >= self.args.max_n_tags, 0)

            dec_out, dec_states = self.model.decoder(decoder_input, src_features, dec_states, tag=decoder_tag, step=step)
            ...
            if(self.args.block_trigram):
                ...
                if(cur_len>3):
                    for i in range(alive_seq.size(0)):
                        fail = False
                        words = [int(w[0]) for w in alive_seq[i]]
            ...
            pre_alive_seq = alive_seq.index_select(0, select_indices.int())
            cur_ids = topk_ids.view(-1, 1, 1)
            cur_tags = pre_alive_seq[:, -1:, 1:] + cur_ids.eq(self.sep_token).long()
            alive_seq = torch.cat([pre_alive_seq, torch.cat([cur_ids, cur_tags], 2)], 1)
            ...
            if is_finished.any():
                predictions = alive_seq.view(-1, beam_size, alive_seq.size(1), alive_seq.size(2))
                ...
                alive_seq = predictions.index_select(0, non_finished) \
                    .view(-1, alive_seq.size(1), alive_seq.size(2))
            # Reorder states.
            select_indices = batch_index.view(-1)
            src_features = src_features.index_select(0, select_indices.int())
            dec_states.map_batch_fn(
                lambda state, dim: state.index_select(dim, select_indices.int()))
