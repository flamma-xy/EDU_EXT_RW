...
class ExtSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint):
        ...
        self._span_extractor = SelfAttentiveSpanExtractor(input_dim = 768)

        if(args.max_pos>512):
            my_pos_embeddings = nn.Embedding(args.max_pos, self.bert.model.config.hidden_size)
            my_pos_embeddings.weight.data[:512] = self.bert.model.embeddings.position_embeddings.weight.data
            my_pos_embeddings.weight.data[512:] = self.bert.model.embeddings.position_embeddings.weight.data[-1][None,:].repeat(args.max_pos-512,1)
            print(my_pos_embeddings)
    ...
    def forward(self, src, segs, clss, mask_src, mask_cls, disco_span):
        segs = (1 - segs % 2) * mask_src.long()
        top_vec = self.bert(src, segs, mask_src)

        disco_mask = (disco_span[:, :, 0] >= 0)
        disco_span = torch.nn.functional.relu(disco_span.float()).long()
        
        disco_span_vec = self._span_extractor.forward(top_vec, disco_span, mask_src, disco_mask.long())
        
        
        # disco_span: shape `(batch_size, num_spans, 2)

        # sents_vec = top_vec[torch.arange(top_vec.size(0)).unsqueeze(1), clss]
        # sent_scores = self.ext_layer(sents_vec, mask_cls).squeeze(-1)
        disco_scores = self.ext_layer(disco_span_vec, disco_mask).squeeze(-1)
        return disco_scores, disco_mask
